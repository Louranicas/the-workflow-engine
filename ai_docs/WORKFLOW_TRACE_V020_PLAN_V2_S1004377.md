...
| Substrate-drift canary itself drifts (V3 self-canary problem) | **Per NA-4 + §6 revised row:** Watcher's deployment-watch journal asserts m16 heartbeat liveness; missing heartbeat for N cycles = substrate-emitted alert. If Watcher integration cannot ship, self-canary problem is **not mitigated** — risk says so honestly. **[v0.2.0 SHIPPED 2026-05-24 honesty amendment, S1004590]** — V3 m16 KEYSTONE shipped (`src/m16_substrate_drift_canary/`, 13 tests, `8757e50`); the canary *types*, *emitter*, and `AlertBudget` rate-limiter (per C-9 new row) all live. **The Watcher liveness assertion DID NOT ship in v0.2.0**: m16 emits a `Heartbeat` envelope per cycle, but no `synthex-v2/m8_watcher/*` module consumes it, so the self-canary loop is NOT closed at v0.2.0. Operator hand-off OP-6 (CHANGELOG `[v0.2.0]`) names this honestly. **The risk is mitigated structurally (the heartbeat exists and is shaped for consumption) but NOT loop-closed.** Closing the loop requires Watcher-side integration in a future round (v0.2.2+ or a coordinated cross-substrate ship). |
