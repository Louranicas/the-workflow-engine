...
| **CC-7** | Pressure-Driven Evolution (E → F, plus E → spec interviews) | **WIRED Phase 7** (Plan v2 § 15 D21/D22/D24). In-code wire: `m15::PressureRegister::read_pressure_level` returns a bounded scalar in `[0.0, 1.0]` derived from the count of outstanding `PHASE-B-RESERVATION-NOTICE` JSONL files (saturated at `PRESSURE_SATURATION_N`); `orchestration::crystallise::run` reads that scalar via `read_pressure_scalar_for_compose` and feeds it through to `m23::compose_proposals_with_pressure`, which stable-sorts proposals by `evidence_lift + pressure_priority_bonus` where the bonus is in `[0.0, MAX_PRESSURE_CONTRIBUTION]` (additive, bounded — D22). Safer variants (`MutationKind::Identity` > `Swap` > `Skip`) surface first under elevated pressure (NA framing — D24, the substrate's voice in composition). Zero pressure ⇒ output IDENTICAL to the pre-Phase-7 `compose_proposals` path. The original file-drop coupling (Watcher ☤ + Zen reading the JSONL ledger asynchronously) is **preserved alongside** the in-code wire. Pressure NEVER bypasses the F2 evidence floor — it modulates order, not gating. Integration test: `tests/cc7_pressure_evolution.rs` (8 tests). |
