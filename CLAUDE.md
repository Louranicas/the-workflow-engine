...
> **Status:** ACTIVE — G9 fired 2026-05-17, HOLD-v2 lifted. The 26-module Rust codebase is implemented (~31k LOC; `workflow_core` lib + `wf-crystallise` / `wf-dispatch` binaries — both now **real CLI programs**, wired in C22). **Hardening Fleet 2026-05-21 (W0–W5) + assessment-driven remediation S1003733 — both COMPLETE.** S1003733 closed 21 of 22 assessment findings across 5 gated waves + the C22 binary wiring (pipeline logic in a new `workflow_core::orchestration` module) + Wave G mutant-killing. **1967 tests passing, clippy + pedantic clean**; commits through `2096fd0` pushed both remotes. Current-authority state: [`CLAUDE.local.md`](CLAUDE.local.md) snapshot + [`GATE_STATE.md`](GATE_STATE.md) (G9 FIRED) + git history. Any "planning-only" / "stub binary" language elsewhere in the repo is superseded archaeology.
